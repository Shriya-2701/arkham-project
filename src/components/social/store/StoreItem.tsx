  ...
  <div className="bg-[#1a0f0f]/90 rounded-lg overflow-hidden border border-[#8B0000]/30 academia-border blood-scroll">
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-xl font-bold text-silver academia-text">
          ${item.price}
        </span>
      </div>

      <p className="text-silver/80 text-sm mb-4 academia-text">
        {item.description}
      </p>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center">
          ...
          />
          <span className="text-silver/60 text-sm ml-2 academia-text">
            {item.seller.name}
          </span>
        </div>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {item.tags.map((tag) => (
          <span
            key={tag}
            className="px-2 py-1 bg-[#0a0505] rounded-full text-xs text-silver/60 academia-text"
          >
            {tag}
          </span>
        ))}
