  ...
  <div className="space-y-6">
    ...
    <div className="space-y-4">
      {adCampaigns.map((campaign) => (
        <div key={campaign.id} className="bg-zinc-900 rounded-lg p-6 border border-zinc-800">
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            ...
            <div className="bg-zinc-800/50 rounded-lg p-4">
              ...
              <div className="text-zinc-200">{campaign.reach.toLocaleString()}</div>
            </div>
            ...
            <div className="bg-zinc-800/50 rounded-lg p-4">
              ...
              <div className="text-zinc-200">{campaign.engagement.toLocaleString()}</div>
            </div>
          </div>
          ...
          <div className="border-t border-zinc-800 pt-4">
            ...
            <div className="flex flex-wrap gap-2">
              <span className="px-2 py-1 bg-zinc-800 rounded-full text-xs text-zinc-400">
                Age: {campaign.targetAudience.age[0]}-{campaign.targetAudience.age[1]}
              </span>
              {campaign.targetAudience.locations.map((location) => (
                <span key={location} className="px-2 py-1 bg-zinc-800 rounded-full text-xs text-zinc-400">
                  {location}
                </span>
              ))}
              {campaign.targetAudience.interests.map((interest) => (
                <span key={interest} className="px-2 py-1 bg-zinc-800 rounded-full text-xs text-zinc-400">
                  {interest}
                </span>
              ))}
            </div>
