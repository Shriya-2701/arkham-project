...
import { Brain, Clock, Heart, Users, Compass, Moon, HourglassIcon, Shuffle, History, Route, MapPin, GraduationCap, Palette } from 'lucide-react';
...
export const AlgorithmSelector = ({ selectedAlgorithm, onAlgorithmChange }: AlgorithmSelectorProps) => {
  return (
    <div className="bg-zinc-900/95 backdrop-blur-sm border border-zinc-800/50 rounded-xl p-6 mb-8">
      <h2 className="text-xl text-zinc-200 mb-6 font-cormorant">Content Algorithm</h2>
          ...
          return (
            ...
            >
              <Icon className={`w-5 h-5 mt-1 ${
                selectedAlgorithm === algorithm.id ? 'text-purple-400' : 'text-zinc-400'
              }`} />
              <div className="text-left">
                <div className={`font-medium ${
                  selectedAlgorithm === algorithm.id ? 'text-purple-400' : 'text-zinc-200'
                }`}>
                  {algorithm.name}
                </div>
                <div className="text-sm text-zinc-400">{algorithm.description}</div>
              </div>
            </button>
          );
        })}
