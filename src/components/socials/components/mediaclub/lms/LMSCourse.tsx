...
export const LMSCourse = ({ course }: LMSCourseProps) => (
  <div className="bg-[#1a0f0f]/90 academia-border rounded-lg overflow-hidden">
    ...
    <div className="p-6">
      <h3 className="text-xl text-[#B8864D] academia-text mb-2">{course.title}</h3>
      <p className="text-[#B8864D]/80 academia-text mb-4">{course.description}</p>
      
      <div className="flex items-center justify-between">
        <div className="flex items-center text-[#B8864D]/60">
          <Award className="w-4 h-4 mr-1" />
          <span className="text-sm">{course.completed ? 'Completed' : `${course.progress}% Complete`}</span>
        </div>
